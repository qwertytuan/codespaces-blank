...
void HienThi_Menu() {
    ...
    cout << "\t\t1. Giu cho chuyen bay\n";
    cout << "\t\t2. Dat cho chuyen bay\n";
    cout << "\t\t3. Huy giu cho\n";
    cout << "\t\t4. Thoat\n";
}
...
//mo file LuuTruMayBay.txt
 ifstream infile;
  ...
  ////////////////////////////////
    ...
    ofstream file2("GiuCho.txt", ios::app);
     if (file2.is_open()) {
...
void Dat_Cho() {
 // tao object chuyen bay la f2
     ...
     class nguoidung u;
    string ten_nguoi_dung;
   ifstream file("GiuCho.txt", ios::in);

    // kiem tra xem file da mo thanh cong chua
    if (file.is_open()) {
        // hien thi thong tin
        cout<<"\t\t\t"<<" Danh sach giu cho:"<<"\n\n";
     ...
     ifstream file_in("GiuCho.txt", ios::in);
...
void Huy_Dat_Cho() {
    ...
    // mo file temp1 de ghi
    ifstream file_in("GiuCho", ios::in);
